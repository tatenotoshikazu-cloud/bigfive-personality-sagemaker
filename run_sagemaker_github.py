"""
AWS SageMaker実行スクリプト（GitHub連携版）
GitHubリポジトリから直接コードを取得してSageMakerでトレーニング実行
"""
import sagemaker
...
class BigFiveSageMakerTrainer:
    ...
    def __init__(
        ...
        print("=" * 60)
        print("SageMaker設定（GitHub連携）")
        print("=" * 60)
        print(f"Region: {self.region}")
        print(f"Bucket: {self.bucket}")
        print(f"Role: {self.role}")
        print(f"GitHub Repo: {self.github_repo}")
        print(f"GitHub Branch: {self.github_branch}")
        print("=" * 60)
    ...
    def train_stage1(
        ...
        print(f"コード取得: {self.github_repo} ({self.github_branch})")
        print("=" * 60)

        estimator = PyTorch(
            entry_point='train_stage1_local.py',
            git_config={
    ...
    def train_stage2(
        ...
        estimator = PyTorch(
            entry_point='train_stage2_local.py',
            git_config={
                'repo': self.github_repo,
                'branch': self.github_branch,
            },
...
def main():
    """メイン実行例"""
    print("=" * 60)
    print("Big Five性格特性推定 - AWS SageMaker実行（GitHub連携）")
    print("=" * 60)
    ...
    # データアップロード
    # 注意: ローカルで data/local/processed/ を事前に作成しておく必要があります
    s3_data_path = trainer.upload_data_to_s3(
        local_data_dir='data/local/processed',
...
if __name__ == '__main__':
    ...
    print("- data/local/processed/ ディレクトリを準備してください")
