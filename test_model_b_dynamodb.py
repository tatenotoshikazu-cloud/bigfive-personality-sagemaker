...
print("=" * 80)
print("Model B テスト - DynamoDBデータで予測")
print("=" * 80)
...
print("\n[2/4] Fetching DynamoDB records...")
dynamodb = boto3.resource('dynamodb', region_name='ap-northeast-1')
table = dynamodb.Table('recording-poc-records')

response = table.scan(Limit=5)  # Get first 5 records
...
for i, record in enumerate(records, 1):
    ...
    if not transcript:
        print(f"\n  Record {i} (uuid: {uuid}): No transcript")
        continue
...
print("\n" + "=" * 80)
print("SUCCESS: DynamoDB予測テスト完了!")
print("=" * 80)
print("\nModel B (Stage 1 + Stage 2, 1エポック) はDynamoDBデータで正常に動作しました。")
print("=" * 80)
