...
"""
ローカルテスト用トレーニングスクリプト
小規模データで1 epochだけ学習し、全フローを検証

検証項目：
1. 学習が正常に進行するか
2. 保存された重みが学習済みか（初期値と異なるか）
3. ロードした重みで推論できるか
4. 異なるテキストで異なる予測が出るか
"""
...
print("=" * 80)
print("LOCAL TRAINING TEST - Big Five Regression")
print("=" * 80)
...
class BigFiveDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        labels = torch.tensor([
            float(example.get(trait, 50.0)) / 100.0  # 0-99 -> 0-1
...
val_dataset_raw = load_from_disk('data/small_bigfive/val')
...
train_dataset = BigFiveDataset(train_dataset_raw, tokenizer)
val_dataset = BigFiveDataset(val_dataset_raw, tokenizer)
...
# 学習前の重みを保存（比較用）
print("\n" + "=" * 80)
print("STEP 3: Saving BEFORE-TRAINING weights for comparison")
print("=" * 80)
...
# 学習
print("\n" + "=" * 80)
print("STEP 4: Training (1 epoch only)")
print("=" * 80)
...
# 学習後の重みを確認
print("\n" + "=" * 80)
print("STEP 5: Verifying weights were ACTUALLY trained")
print("=" * 80)
...
if weight_diff < 0.0001:
    ...
    print(f"\nSUCCESS: Weights changed by {weight_diff:.6f} (training worked!)")
...
print(f"\nSaved checkpoint regressor weight mean: {saved_weight.mean():.6f}")
...
if saved_diff < 0.0001:
    print("\nERROR: Saved checkpoint contains INITIAL weights, not trained weights!")
    sys.exit(1)
else:
...
# 新しいモデルインスタンスにロード
print("\n" + "=" * 80)
print("STEP 7: Loading saved model into NEW model instance")
print("=" * 80)
...
# 推論テスト
print("\n" + "=" * 80)
print("STEP 8: Testing inference with DIFFERENT texts")
print("=" * 80)

test_texts = [
    "I am very happy and excited today! Life is wonderful and I love meeting new people.",
    "I always organize my tasks carefully and complete everything on time. I am very responsible.",
    "I prefer to stay quiet and work alone. Social situations make me uncomfortable.",
]
...
# 予測の多様性を確認
print("\n" + "=" * 80)
print("STEP 9: Verifying predictions are DIFFERENT for different texts")
print("=" * 80)
...
for i in range(len(BIG5_LABELS)):
    ...
    if std < 0.01:
        print(f"  WARNING: {trait} predictions are almost identical!")
...
if avg_std < 0.1:
    print(f"\nERROR: Predictions are too similar (avg std: {avg_std:.2f})")
    print("Model is not actually using the input text!")
    sys.exit(1)
else:
    print(f"\nSUCCESS: Predictions vary across different texts (avg std: {avg_std:.2f})")
